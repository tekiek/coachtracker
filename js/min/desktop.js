...
app['timer'] = new function() {
	...
	this.init = function() {
		EventManager.observe('loadFirstScreen', _timer.loadTime);
	}
	...
	this.loadTime = function() {
		...
		if (loadStart) {
			_timer.timers[loadTimeId] = {};
			_timer.timers[loadTimeId]['start'] = loadStart;
			_timer.endTimer(loadTimeId);
		}
	}

	this.startTimer = function(id) {
		_timer.timers[id] = {};
		_timer.timers[id]['start'] = new Date().getTime();
	}

	this.endTimer = function(id) {
		if (_timer.timers[id]) {
			_timer.timers[id]['end'] = new Date().getTime();
			_timer.timers[id]['time'] = _timer.timers[id]['end'] - _timer.timers[id]['start'];
			
			// Find divisible
			var mod = 1;
			$.each([1000, 100, 10], function(x, i) { 
				if (_timer.timers[id]['time'] >= i) {
					mod = i;
					return false;
				}
			});
			...
			// Track timer
			timeTrack = Math.round(_timer.timers[id]['time'] / mod) * mod;
...
app['global'] = new function() {
	...
	this.removeHTML = function(string) {
	...
	this.dialog = function(params) {
		...
		if (!params['transition']) params['transition'] = 'slideInDown';
		if (!params['transitionSpeed']) params['transitionSpeed'] = 'default';
		...
		$(_global.els['dialogForm']).center();
			
		if (params['animate']) {
			app.global.animate(_global.els['dialogForm'], params['transition'], params['transitionSpeed'], function() {
	...
	this.dialogConfirm = function(params) {
		...
		if (!params['title']) params['hideTitle'] = true;
	...
	this.animate = function(el, animation, speed, callback) {
		...
		console.log('Animate:', animation, speed);
		$(el)
	...
	this.alert = function(params) {
		...
		var alert = _global.els.alert,
			icon = (params['icon'] ? $.tmpl(app.templates.icon, { 
				classes: 'fa-lg fa-white', 
				icon: params['icon'] 
			}) : ''),
...
app['libs'] = new function() {
	_libs = this;
	this.loaded = [];
	
	this.init = function() {
		EventManager.observe('lib:loaded', this.libLoaded);
	}
	
	this.libLoaded = function(params) {
		if (!params) params = {};
		if (!params.lib) params.lib = "";

		_libs.loaded.push(params.lib);
		EventManager.fire(params.lib + ":loaded");
	}
	
	this.isLoaded = function(lib) {
		return ($.inArray(lib, _libs.loaded) >= 0 ? true : false);
	}
	
	this.waitForLib = function(params) {
		if (!params) params = {};
		if (!params.lib) params.lib = "";
		if (!params.cbParams) params.cbParams = {};
		
		if (_libs.isLoaded(params.lib)) {
			if (params.cb) params.cb(params.cbParams);
		} else {
			$(document).overlay({ show: true });
			EventManager.observe_once(params.lib + ":loaded", function() {
				$(document).overlay({ show: false });
				if (params.cb) params.cb(params.cbParams);
			});
		}
...
app['templates'] = {
	'svg'			: '<span class="${classes}"><svg role="img" viewBox="0 0 1792 1792"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#${icon}"></use></svg></span>',
	'input'			: '<input data-regex="${regex}" data-action="${gtAct}" name="${name}" value="${value}" data-field="${dbId}" data-valid="${validLength}" type="${inputType}" class="form-control ${classes}" placeholder="${placeholder}" data-mask="${mask}">',
	'textarea' 		: '<textarea data-action="${gtAct}" data-field="${dbId}" data-valid="${validLength}" class="form-control ${classes}" placeholder="${placeholder}">${value}</textarea>',
	...
	'multi'			: '<div data-type="MULTI" data-field="${dbId}" class="form-control multi" data-valid="${validLength}" ></div>',
	'multi_option'	: '<input data-action="${gtAct}" type="checkbox" name="${key}" id="${key}"><label class="col-xs-12 col-sm-6 col-md-6 col" for="${key}">${value}</label>',
	'field_wrapper'	: '\
		<div class="input-group input-group-lg ${classes}">\
	...
	'field'			: '<div class="input-group input-group-lg ${classes}"></div>',
	'field_addon'	: '\
		<span class="input-group-addon">\
			<span class="${classes}">\
				<svg role="img" viewBox="0 0 1792 1792">\
	...
	',
	'button'	: '\
		<button data-action="${text}" class="btn ${color} ${btnSize} ${classes} ${acls}">\
			<svg role="img" viewBox="0 0 1792 1792">\
				<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#${icon}"></use>\
			</svg>\
			<span class="button-text">${text}</span>\
		</button>',
	'icon'		: '<span class="${classes}" data-action="${gtAct}" >\
					<svg role="img" viewBox="0 0 1792 1792">\
						<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#${icon}"></use>\
					</svg>\
				 </span>',
	...
	'alert'		: '<div class="alert alert-danger" role="alert">${text}</div>',
...
app['emailBlast'] = new function() {
	_emailBlast = this;
	this.schools;
	this.data = {
		studentFields: ['cbo', 'college'],
		totalFields: []
	};
	this.els = {};
	this.apis = {
	...
	this.template_data = {
		'saveEvent': {
			...
			'iconSize'		: 'fa-lg',
			'text'			: 'SAVE!',
			'classes'		: 'marginBottom10 marginTop'
		}
	}
	
	this.init = function() {
		_emailBlast.getEls();
		_emailBlast.setupHeader();
		_emailBlast.addFields();
		_emailBlast.addSaveBtn();
	}
	
	this.getEls = function() {
		_emailBlast.els = {
			parent: $('div#email-blast')
		}
	}
	...
	this.getStudentGroups = function() {
		var userStudents = app.data.user.students,
			fields = _emailBlast.data.studentFields,
			totalFields = [];

		// Loop throught students
		$.each(userStudents, function(x, student) {

			// Loop through field list
			$.each(fields, function(y, field) {
				var studentField = student[field];

				// If unique add to list
				if (studentField && $.inArray(studentField, _emailBlast.data.totalFields) == -1) {
					_emailBlast.data.totalFields.push(student[field]);
				}
			});
		});

		// Sort Alpha
		_emailBlast.data.totalFields.sort();

		// Add All Students
		_emailBlast.data.totalFields.unshift('All Students');
	}

	this.addFields = function() {
		var eventFieldData = app['fieldController'].getFields(app['fieldsEmailBlast']);

		// Get Student Groups
		_emailBlast.getStudentGroups();
		eventFieldData.studentid.options = _emailBlast.data['totalFields'];
		
		$.each(eventFieldData, function(key, field) {
			var fieldWrapper = $.tmpl(app.templates.field_wrapper, field),
				fieldEl = app.fieldController.createField(field);
				_emailBlast.els[key] = new Object();
		
			// Add to Dom
			fieldWrapper.append(fieldEl);
			_emailBlast.els['parent'].append(fieldWrapper);
		
			// Store Els
			_emailBlast.els[key]['wrapper'] = $(fieldWrapper);
			_emailBlast.els[key]['field'] = $(fieldEl);
		});
	}
	...
	this.addSaveBtn = function() {
		var saveEvent = $.tmpl(app.templates['button'], _emailBlast.template_data['saveEvent']);
		
		// Add to DOM
		_emailBlast.els['parent']
			.append($.tmpl(app.templates.hr))
			.append(saveEvent);
		_emailBlast.els['saveEvent'] = saveEvent;
		
		// Add Events
		_emailBlast.els['saveEvent'].click(_emailBlast.saveEvent);
	}
	...
	this.getStudentIdsFromGroup = function(groupId) {
		var selectedField = _emailBlast.data['totalFields'][groupId],
			userStudents = app.data.user.students,
			fields = _emailBlast.data.studentFields,
			studentid = [];
		...
		$.each(userStudents, function(x, student) {
			if (groupId == 0) {
				...
				$.each(fields, function(y, field) {
					if (student[field] == selectedField) {
						studentid.push(student.id);
					}
				});
			}
	...
	this.saveExit = function(params) {
		var data = _emailBlast.getFieldData();

		// Get student ids
		data['studentid'] = _emailBlast.getStudentIdsFromGroup(data['studentid']);

		// Set static values
		data = $.extend({}, data, {
			duration: '0M',
			location: 'Email Blast',
			userid: app.data.user.id
		});

		$.ajax({
			type: "POST",
			url: _emailBlast.apis.saveEvent,
			data: data,
			dataType: 'json',
			offline: true
		})
	...
	this.saveEvent = function() {
		if (!_emailBlast.validateFields()) return false;

		app.global.dialogConfirm({
			msg: 'Would you like to save this email blast?',
			saveCloseCallback: function() {
				_emailBlast.saveExit();
			}
		});
	}
};
...
app['fieldsEmailBlast'] = {
	...
	'notes': {
		'type'			:'textarea',
		'placeholder'	:'Paste email message here...',
		'mask'			: null,
		'dbId'			: 'notes',
		'validLength'	: 0,
		'icon'			:'comment-o',
...
var supportsTransitions = null; //document.body.style.transition != null
...
try {
	...
	console.log('ERROR: app libs not loaded');
}
/*!
 * jQuery Form Plugin
